...
@pytest.mark.asyncio
async def test_list_course_journals_endpoint(
    client: AsyncClient,
    db_session: AsyncSession,
    test_admin: User,
    test_mentor: User,
    ...
    response = await client.get(f"/api/v1/courses/{course.id}/journals", headers=admin_headers)
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 2
    assert data[0]["period_label"] == "Week 1"
    assert data[0]["period_type"] == "week"
    assert data[1]["period_label"] == "Week 2"

    # A student not enrolled in the course is forbidden.
    student_token = create_access_token(test_student.id, test_student.role)
    student_headers = {"Authorization": f"Bearer {student_token}"}
    forbidden_response = await client.get(f"/api/v1/courses/{course.id}/journals", headers=student_headers)
    assert forbidden_response.status_code == 403

    # An enrolled student can view the list.
    enrollment = Enrollment(
        student_id=test_student.id,
        course_id=course.id,
        price_at_enrollment=Decimal("150.0"),
    ...
    allowed_response = await client.get(f"/api/v1/courses/{course.id}/journals", headers=student_headers)
    assert allowed_response.status_code == 200
    assert len(allowed_response.json()) == 2
