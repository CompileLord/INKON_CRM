...
4. При сохранении проверяется: `exam_score <= journal.exam_max_score` и `bonus_score <= 20`.
...
- Exam больше `journal.exam_max_score` (например, 80 при максимуме 70) → 400 «Exam score cannot exceed journal maximum».
- Bonus = 25 при лимите 20 → 400 «Bonus score cannot exceed 20».
