...
export function ActivityFeed() {
  ...
  const getActionIcon = (action: string) => {
    switch (action) {
      ...
      case "DELETE":
        return <History size={14} className="text-rose-600" />;
      default:
      ...
      <div className="flex-1 px-5 py-4">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-6 text-center">
            <p className="text-xs text-rose-600 font-semibold">{t("errorLoading", "Ошибка загрузки активности")}</p>
            <button
              onClick={() => refetch()}
              className="mt-2 flex items-center gap-1.5 rounded-lg border border-border px-3 py-1 text-xs font-medium text-ink hover:bg-cream"
            >
            ...
            {logs.map((item, index) => {
              ...
              return (
                <li key={item.id} className="relative flex gap-3 pb-3 last:pb-0">
                  ...
                  <span className="relative z-10 flex h-[30px] w-[30px] shrink-0 items-center justify-center rounded-full border border-border bg-card">
                    {getActionIcon(item.action)}
                  </span>
                  <div className="flex-1 rounded-lg border border-event-border bg-strip px-4 py-2.5">
                    <p className="text-xs font-semibold text-ink">
                      {item.user_name || "User"} • {item.action} {item.entity_type} #{item.entity_id}
                    </p>
                    {item.field_name && (
