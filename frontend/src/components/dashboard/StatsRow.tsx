...
export function StatsRow() {
  ...
  return (
    <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
      <div className="relative flex h-30 flex-col justify-between overflow-hidden rounded-xl border border-border-warm bg-beige-dark p-5">
        ...
        <p className="relative text-[26px] font-bold leading-none text-ink">{totalDebt}</p>
        <span className="relative inline-flex w-fit items-center rounded-full bg-rose-100 px-2.5 py-0.5 text-xs font-semibold text-rose-800">
          {debtorsCount}
        </span>
      </div>

      <div className="flex h-30 flex-col justify-center rounded-xl border border-border-warm bg-card p-5">
        <BookOpen size={22} className="text-maroon" strokeWidth={2} />
        <p className="mt-2 font-bold text-ink text-lg">{activeCoursesCount} {t("activeCourses")}</p>
        <p className="mt-0.5 text-xs text-muted">{t("activeCoursesSubtitle")}</p>
      </div>

      <div className="flex h-30 flex-col justify-center rounded-xl border border-border-warm bg-card p-5">
        <Users size={22} className="text-blue-600" strokeWidth={2} />
        <p className="mt-2 font-bold text-ink text-lg">{activeStudentsCount} {t("registeredStudents")}</p>
